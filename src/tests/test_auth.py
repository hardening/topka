...
class Test(unittest.TestCase):
    ...
    def testCascade(self):
        ...
        self.assertIsInstance(c.authenticate("login2", "dom", "password in p1", props), AuthContext)
        self.assertIsInstance(c.authenticate("login2", "dom", "password in p2", props), AuthContext)
        
        # test unique login
        c = CascadingProvider((p1, p2), True)
        self.assertIsInstance(c.authenticate("login2", "dom", "password in p1", props), AuthContext)
        self.assertIsNone(c.authenticate("login2", "dom", "password in p2", props))
        
    
    def testMap(self):
        p = DomainMapProvider({"yes": YesProvider(), "no": NoProvider()})
        
        self.assertIsNone(p.authenticate("login", "no", "", props), "no domain")
        self.assertIsInstance(p.authenticate("login", "yes", "", props), AuthContext, "yes domain")

        # ship a default provider
        p = DomainMapProvider({"yes": YesProvider(), "no": NoProvider()}, YesProvider())
        self.assertIsInstance(p.authenticate("login", "not yes, not no", "", props), AuthContext)
